...
import {checkGeminiAPI, fetchAvailableGeminiModels, setGeminiAPI} from "./geminiManager";
		...
		if (settings.geminiApiKey) {
			try {
				...
				if (settings.availableGeminiModels.length > 0) {
					...
					if (!modelExists) {
						settings.geminiModel = settings.availableGeminiModels[0];
					}
				}
...
export class AIImageAnalyzerSettingsTab extends PluginSettingTab {
	...
	displayGeminiSettings(containerEl: HTMLElement): void {
		...
		new Setting(containerEl)
			...
			.setDesc('Test the connection to the Gemini API and fetch available models')
			.addButton(button => {
	...
	private setupFetchModelsButton(button: ButtonComponent): void {
		button.setButtonText(this.fetchingModels ? 'Fetching Models...' : 'Test & Fetch Models');
	...
	private async fetchAvailableModels(apiKey: string): Promise<void> {
		...
		try {
			...
			if (models.length > 0) {
				const modelExists = models.some(model => model.model === settings.geminiModel.model);
				if (!modelExists) {
			...
			if (this.geminiModelsDropdown) {
				this.geminiModelsDropdown.setDesc('Select the Gemini model to use');
			}
		}
	}
	...
	private updateGeminiModelDropdown(): void {
		...
		const modelsToUse = settings.availableGeminiModels.length > 0 
		...
		this.geminiModelsDropdown.addDropdown(dropdown => {
			...
			const options = modelsToUse.reduce((acc, model) => {
			...
			dropdown.onChange(async (value) => {
				const selectedModel = modelsToUse.find(model => model.model === value);
				if (selectedModel) {
